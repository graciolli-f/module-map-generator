  ...
  _processModule(filePath, moduleInfo) {
    ...
    if (moduleInfo.imports) {
      for (const imp of moduleInfo.imports) {
        ...
        const isNodeBuiltin = ['fs', 'path', 'child_process', 'util', 'events', 'os', 'crypto', 'http', 'https', 'stream', 'buffer', 'querystring', 'url', 'zlib'].includes(imp.source) || 
                             imp.source.startsWith('node:');
        
        if (isRelative) {
          // Internal project import - ALWAYS track these
          ...
          // Only track external deps if configured to do so
          if (isNodeBuiltin) {
